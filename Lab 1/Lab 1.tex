...
Таким образом, можно обнаружить, что 


$1! +2! +3! + 4!...+100! \equiv  1! +  +2! +3! + 0 + ...+ 0 \equiv 9 \mod(12) $

Из этого следует, что остаток от деления данной суммы на 12 равен 9.
 
 В последней теореме было доказано, что если  $a \equiv b \pmod{n}$ , то ca = cb (mod n) для любого c. 
...
\begin{theorem}{$4-3$}
 		Если $ca \equiv cb \pmod n$, то $a\equiv b \pmod {\dfrac{n}{d}}$, где d = НОД (c,n)
 		
 
\end{theorem}

\begin{proof}
 		Предположим, что $$c(a-b)=ca-cb=kn$$  для некоторого k. Зная, что НОД (с,n) = d, можно сделать вывод, что существуют простые числа r и s, удовлетворяющие равенствам $c = dr, n = ds$. Когда эти значения подставляются в уравнение, d сокращается и получается следующее: $$r(a-b)=ks$$
 		
 		Следовательно, $s\mid r(a-b)$ и НОД(r,s) = 1. Лемма Евклида подразумевает, что $s\mid a-b$, которое может быть переписано в виде $a \equiv b \pmod{n}$; другими словами, $a\equiv b \pmod {\dfrac{n}{d}}$
 		
 

\end{proof}

Теорема 4-3 наиболее полезна, когда добавляется условие: НОД(с,n) = 1, тогда сокращение может быть выполнено без изменения модуля.

\begin{corollary}
	Если $ca \equiv cb \pmod n$ и НОД(c,n) = 1, то  $a \equiv b \pmod n$
\end{corollary}
Мы воспользуемся моментом, чтобы записать особый случай следствия 1, которым мы будем часто пользоваться, а именно:

\begin{corollary}
	Если $ca \equiv cb \pmod p$ и p не делит с (p - простое число), то $a \equiv b \pmod p$
\end{corollary}
